...
- Code implementation: M4 contract-hardening batches are active after M4 plan filing and design
...
- Allowed work now: M4-C core lifecycle and ownership contract tests through TDD implementation
...
| GQ-080 | Execute M4-C core lifecycle and ownership contract tests | GQ-079 | in_progress | requires TDD implementation subagent and independent audit; must not call heap-owned runtime handles after successful destroy |
...
GQ-078 through GQ-084, with GQ-080 as the current execution point. No post-M4 implementation
