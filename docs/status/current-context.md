...
- M3 Core Middleware Contract And Native In-Process Backend is active under a user-approved plan:
  ...
  the active implementation batch. M3-D audit recorded a non-blocking `runtime.c` create-failure
  cleanup hardening note that should not be forgotten when later backend failure paths become
  testable.
